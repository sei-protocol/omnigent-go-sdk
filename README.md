...
`Go SDK generated bindings` CI job pins both files byte-for-byte to
`spec/openapi.json` by regenerating into a temp directory and comparing. Nothing
pins that spec to a live server, so the chain is one link shorter than it looks —
`spec/README.md` records where the snapshot came from and how to refresh it. The
check fails when `oapi-codegen` is absent instead of skipping;
...
- **`Go SDK tests`**, over the module's declared floor and current stable, runs
  `bin/check.sh` — gofmt, `go build`, `go vet`, `go test -race`,
  `go mod tidy -diff`. That is the same script a contributor runs directly, so
  local and CI cannot drift.
- **`Go SDK generated bindings`** verifies `models.gen.go` and `events.gen.go`
  still match `spec/openapi.json`. Its own job because it needs python and
  `oapi-codegen`, which `bin/check.sh` deliberately does not depend on, and
  pinned to a fixed Go line because it compares generated bytes.
- **`Go SDK lint`** runs golangci-lint at a pinned version.

A job that runs is not a job that gates. Each check name has to be in the
branch's required-status-checks list, and the test matrix renders one name per
leg — `Go SDK tests (1.24.x)`, `Go SDK tests (stable)`,
`Go SDK generated bindings`, `Go SDK lint`. That list is repository settings,
not something a pull request can change.
