...
//
// [APIError.Error] renders the status, the error code and message, and
// RequestID — what a bug report needs. It never renders Body. A non-2xx body
// need not come from this API at all: an auth proxy answering 302-to-login or
// 502 can put a CSRF token, a signed state parameter or an echoed request header
// in it, and Error's return value is the one string that reliably reaches a log
// aggregator. Body is still here for the caller who needs to see it, and callers
// who log it choose to.
//
// Code and Message are parsed out of that same body, so they carry the same
// caveat one step weaker: they are populated only when the body matched an
// envelope this API defines, which a foreign responder usually does not produce,
// and what reaches the log is then two named strings rather than opaque bytes. A
// proxy that happens to answer in the same shape is rendered — bounded to its
// code and message.
//
...
func (e *APIError) Error() string {
	...
	case e.Code != "" && e.Message != "":
		detail = fmt.Sprintf("%s: %s: %s", label, e.Code, e.Message)
	case e.Message != "":
...
// bodyPreview renders body for an error message, bounded and rune-safe.
